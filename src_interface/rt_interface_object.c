...
/*   Updated: 2020/11/01 20:30:27 by mperseus         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	info_object_1(t_scene *scene, t_mlx *mlx, int x, int y)
{
	...
	"OBJECT CONTROL");
	mlx_string_put(mlx->mlx, mlx->win, x, y, TEXT_COLOR, "Object:");
	...
	if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_PLANE)
		mlx_string_put(mlx->mlx, mlx->win, x + 210, y, TEXT_COLOR, "Plane");
	else if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_SPHERE)
		mlx_string_put(mlx->mlx, mlx->win, x + 210, y, TEXT_COLOR, "Sphere");
}
...
void	info_object_2(t_scene *scene, t_mlx *mlx, int x, int y)
{
	...
	if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_CONE)
		mlx_string_put(mlx->mlx, mlx->win, x + 210, y, TEXT_COLOR, "Cone");
	else if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_CYLINDER)
		mlx_string_put(mlx->mlx, mlx->win, x + 210, y, TEXT_COLOR, "Cylinder");
