...
/*   Updated: 2020/11/14 05:55:25 by wquirrel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	info_object_1(t_scene *scene, t_mlx *mlx, int x, int y)
{
	...
	if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_PLANE)
		mlx_string_put(mlx->mlx, mlx->win, x + 260, y, TEXT_COLOR, "Plane");
	else if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_SPHERE)
		mlx_string_put(mlx->mlx, mlx->win, x + 260, y, TEXT_COLOR, "Sphere");
	else if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_CYLINDER)
		mlx_string_put(mlx->mlx, mlx->win, x + 260, y, TEXT_COLOR, "Cylinder");
	else if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_CONE)
		mlx_string_put(mlx->mlx, mlx->win, x + 260, y, TEXT_COLOR, "Cone");
}
...
void	info_object_2(t_scene *scene, t_mlx *mlx, int x, int y)
{
	...
	if (scene->objs.arr[scene->act_obj]->type == OBJECT_TYPE_PARABOLOID)
		mlx_string_put(mlx->mlx, mlx->win, x + 260, y, TEXT_COLOR,
		"Paraboloid");
